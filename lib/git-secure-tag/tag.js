...
Tag.prototype.verify = function verify() {
  ...
  if (match === null) {
    process.stderr.write(
        'error: No `Secure-Tag-V0: ...` found in tag description\n');
    process.exit(1);
    return;
  }
  ...
  hash.calculate(this.dir, (err, hash) => {
    ...
    if (hash === match[1]) {
      process.stderr.write(p.stderr.toString());
      process.stderr.write('Good Secure-Tag hash\n');
      process.exit(0);
      return;
    }

    process.stderr.write('error: Secure-Tag hash mismatch\n' +
                         `error: Expected: ${hash}\n` +
                         `error: Found: ${match[1]}\n`);
    process.exit(1);
  });
...
Tag.prototype.sign = function sign() {
  ...
  hash.calculate(this.dir, (err, hash) => {
    ...
    const msg = (this.options.m ?  this.options.m + '\n' : '') +
                'Secure-Tag-V0: ' + hash;
