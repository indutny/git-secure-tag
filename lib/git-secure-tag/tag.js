...
Tag.prototype.verify = function verify(callback) {
  const args = this.options.insecure ?
      [ 'show', this.options._[0] ] : [ 'verify-tag', '-v', this.options._[0] ];
...
Tag.prototype.sign = function sign() {
  const hash = new (this.options.legacy ? gst.LegacyHash : gst.Hash)(
      this.options);
  hash.calculate(this.dir, this.options._[1], (err, hash) => {
    ...
    const name =
        this.options.legacy ? 'Git-Secure-Tag-V0' : 'Git-EVTag-v0-SHA512';
